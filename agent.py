...
class Agent:
    ...
    # ------------------------------------------------------------------
    # Q-model training on real transitions
    # ------------------------------------------------------------------

    def train_q_model_on_real(self, batch_size):
        """One Double DQN update on real transitions, encoded via RSSM hidden states."""
        ...
        with torch.no_grad():
            states      = self.world_model.encode_observation_to_hidden(self.normalize_observation(obs))
            next_states = self.world_model.encode_observation_to_hidden(self.normalize_observation(next_obs))
    ...
    def train(self, episodes=1, offline_training_epochs=1, batch_size=1,
        ...
        for episode in range(episodes):
            ...
            for _ in range(offline_training_epochs):
                ...
                # Q-model updates on imagined trajectories + real transitions
            ...
            avg_real_q_loss   = total_real_q_loss   / safe_q_denom
